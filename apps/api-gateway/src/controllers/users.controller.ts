...
export async function getMe(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    const result = await pool.query(`SELECT id, phone_number, display_name, avatar_url, created_at FROM users WHERE id = $1`, [req.userId]);
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE users SET display_name = COALESCE($1, display_name), avatar_url = COALESCE($2, avatar_url), updated_at = NOW()
       WHERE id = $3 RETURNING id, phone_number, display_name, avatar_url`,
